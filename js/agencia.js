...
var Agencia = {
	...
	'mostrarAgenciaModelos':function(){
		$.ajax({
			...
			success:function(data){
				...
				$(Agencia.options.modeloscontainer).html(data);
				$(Agencia.options.agenciaModeloModal).appendTo("body").modal('show');
	...
	'cargar_modelos':function(){
		$.ajax({
			url:Agencia.options.modelos_agencia_url,
			type:"GET",
			success:function(data){
				CRContactos_Manager.check_errors(data);					
				$(Agencia.options.agenciaModelosContainer).html(data);
			}
		});
	},
	'asignar_modelo_m':function(){
		$.ajax({
			url: Agencia.options.asignarModeloAgenciaUrl,
			data:$(Agencia.options.agenciaModelosForm).serializeHash(),
			type:'GET',
			success: function(data){				
				CRContactos_Manager.check_errors(data);
				if(data.is_error == 1){
	...
	'remover_modelo':function(maid){		
		$.ajax({
			url:Agencia.options.removerModeloAgenciaUrl,
			data:{
				maid: maid
			},
			type:"GET",
	...
	'mostrarAgenciaNotificaciones':function(){
		$.ajax({
			url:Agencia.options.listaNotificacionesUrl,
			type:"GET",
			success:function(data){
				CRContactos_Manager.check_errors(data);					
				$(Agencia.options.notificacionescontainer).html(data);
				$(Agencia.options.agenciaNotificacionesModal).appendTo("body").modal('show');
	...
	'cargar_notificaciones':function(){
		$.ajax({
			url:Agencia.options.notificaciones_agencia_url,
			type:"GET",
			success:function(data){
				CRContactos_Manager.check_errors(data);					
				$(Agencia.options.agenciaNotificacionesContainer).html(data);
			}
		});
	},
	'asignar_notificacion':function(){
		var fd = $(Agencia.options.agenciaNotificacionesForm).serializeHash();	
		$.ajax({
			url: Agencia.options.agregarAgenciaNotificacionUrl,
			data: fd,
			type:'GET',
			success: function(data){		
				CRContactos_Manager.check_errors(data);
	...
	'remover_notificacion':function(nid){
		$.ajax({
			url:Agencia.options.removerNotificacionAgenciaUrl,
			data:{
				nid: nid
			},
			type:"GET",
	...
	'init':function(){
		...
		Agencia.cargar_administradores_box();
		//Modals
		$(Agencia.options.agenciaModeloModal).on('hidden', function () {
			Agencia.cargar_modelos_box();
	     });
		...
		$(document).on("click",Agencia.options.abreAgenciaNotificacionesModalBtn,function(){
			Agencia.mostrarAgenciaNotificaciones();
		});
		
		$(document).on("click",Agencia.options.removerModeloAgenciaBtn,function(event){
			event.stopImmediatePropagation();
			Agencia.lid = "#"+$(this).attr("lid");
		...
		$(document).on("click",Agencia.options.asignarNotificacionBtn,function(){		
			//event.stopInmediatePropagation();
		Agencia.asignar_notificacion();
		});
		
		$(document).on("click",Agencia.options.asignarModelosBtnB,function(event){
			event.stopImmediatePropagation();
			Agencia.asignar_modelo_m();
		});
		
		$(document).on("click",Agencia.options.abreAgenciaModelosModalBtn,function(){
