 ...
 */
	public $validate = array(
		...
		'temppassword' => array(
			...
			'numeric' => array(
				'rule' => array('numeric'),
				'message' => 'Únicamente caracteres numéricos.'
			),
			'length' => array(
				'rule' => array('between', 7, 10),
				'message' => 'Debe ser de 8 caracteres.'
			),
			'isUnique' => array(
				'rule' => array('isUnique', 'telefono'),
				'message' => 'Este número de teléfono está en uso.'
			)
		),/*,
		'tos' => array(
			'rule' => array('custom','[1]'),
...
public $hasOne = array('Administradore','Vendedore');
 ...
 */
	...
	public function changeUserPassword($postData = array()){
		//$this->validate = $this->validatePasswordChange;

		$this->set($postData);
		//if ($this->validates()) {
			$this->data[$this->alias]['password'] = $this->hash($this->data[$this->alias]['password'], null, true);
			//$this->data[$this->alias]['id'] = post
//print_r($this->data);
//exit;			
