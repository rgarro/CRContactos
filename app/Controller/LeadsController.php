...
class LeadsController extends CrcController {
	...
	public function csv_downloads(){
		if($_GET['reporte']==1){
			$title = 	"Marcas".$_SESSION['reporte_marca_result']['desde']."-".$_SESSION['reporte_marca_result']['hasta']."\n";
      ...
      header('Content-Type: application/vnd.ms-excel');
		...
		if($_GET['reporte']==2){
			$title = 	"Modelos: ".$_SESSION['reporte_modelo_result']['desde'].",".$_SESSION['reporte_marca_result']['hasta']."\n";
      $objPHPExcel = new PHPExcel();
      $objPHPExcel->getProperties()->setCreator("CRContactos")
        ->setLastModifiedBy("Rolando Garro")
        ->setTitle($title)
        ->setSubject("CRCContactos Reporte Modelos")
        ->setDescription("CRContactos Reporte Modelos fechas")
        ->setKeywords("CRContactos")
        ->setCategory("CRContactos");
      ...
      $num = 2;
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('C'.$num, $r['total']);
				foreach($r['modelos'] as $m){
					//$csv_str .= $m['modelo'].",".$m['total']."\n";
          $objPHPExcel->getActiveSheet()->setCellValue('A'.$num, " ");
          $objPHPExcel->getActiveSheet()->setCellValue('B'.$num, $m['modelo']);
          $objPHPExcel->getActiveSheet()->setCellValue('C'.$num, $m['total']);
          $num++;
	 			}
        $num++;
			}
      header('Content-Type: application/vnd.ms-excel');
      //header('Content-Disposition: attachment;filename=\"'.$title.'\"');
      header('Content-Disposition: attachment;filename="crcontactos-reporte-modelo'.'.xls"');
      header('Cache-Control: max-age=0');
      // If you're serving to IE 9, then the following may be needed
      header('Cache-Control: max-age=1');
      $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
	...
	if($_GET['reporte']==3){
			$title = 	"Vendedor: ".$_SESSION['reporte_vendedor_result']['desde'].",".$_SESSION['reporte_vendedor_result']['hasta']."\n";
			//$csv_str .= "nombre,asignado,activo,archivado,vendido,total\n";
      $objPHPExcel = new PHPExcel();
      $objPHPExcel->getProperties()->setCreator("CRContactos")
        ->setLastModifiedBy("Rolando Garro")
        ->setTitle($title)
        ->setSubject("CRCContactos Reporte Vendedor")
        ->setDescription("CRContactos Reporte Vendedor fechas")
        ->setKeywords("CRContactos")
        ->setCategory("CRContactos");
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('B1', "Asignado");
        $objPHPExcel->getActiveSheet()->setCellValue('C1', "Activo");
        $objPHPExcel->getActiveSheet()->setCellValue('D1', "Archivado");
        $objPHPExcel->getActiveSheet()->setCellValue('E1', "Vendido");
      ...
      $num = 2;
        ...
        $objPHPExcel->setActiveSheetIndex(0);
        //$csv_str .= $r['marca'].",".$r['total']."\n";
        $objPHPExcel->getActiveSheet()->setCellValue('A'.$num, $r['User']['nombre']);
        $objPHPExcel->getActiveSheet()->setCellValue('B'.$num, $r['User']['asignado']);
        $objPHPExcel->getActiveSheet()->setCellValue('C'.$num, $r['User']['activo']);
        $objPHPExcel->getActiveSheet()->setCellValue('D'.$num, $r['User']['archivado']);
        $objPHPExcel->getActiveSheet()->setCellValue('E'.$num, $r['User']['vendido']);
        $objPHPExcel->getActiveSheet()->setCellValue('F'.$num, $r['User']['total']);
        $num++;
      }
      header('Content-Type: application/vnd.ms-excel');
      //header('Content-Disposition: attachment;filename=\"'.$title.'\"');
      header('Content-Disposition: attachment;filename="crcontactos-reporte-vendedor'.'.xls"');
      header('Cache-Control: max-age=0');
      // If you're serving to IE 9, then the following may be needed
      header('Cache-Control: max-age=1');
      $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
    ...
    if($_GET['reporte']==4){
        ...
        $objPHPExcel->getProperties()->setCreator("CRContactos")
          ...
          ->setDescription("CRContactos Reporte:#-".date("D, d M Y H:i:s"))
          ->setKeywords("CRContactos Reporte")
          ->setCategory("CRContactos Reporte");
      ...
      header("Content-Disposition: attachment; filename=reporte".date("F-j-Y-g:i-a").".xls");
      header('Cache-Control: max-age=0');
      // If you're serving to IE 9, then the following may be needed
      header('Cache-Control: max-age=1');
      $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
