...
foreach($administradores as $v){ 
	if($v['User']['id'] > 0){	
//here administradores	
?>
    <tr>
        <td><a href="/index.php/users/users/pre_edit?user_id=<?php echo $v['User']['id'];?>"><i class="fa-icon-user"></i> <?php echo $v['User']['nombre'];?> <?php echo $v['User']['apellido'];?></a><br>
            <small><i class="fa-icon-phone"></i> <?php echo $v['User']['telefono'];?></small></td>
									<td class="center"><small><i class="fa-icon-calendar"></i> <?php echo $v['User']['created'];?></small></td>
									<td class="center theclass">
...
<button type="button" user_id="<?php echo $v['User']['id'];?>" class="btn btn-success btn-mini change-user-pass"><i class="fa-icon-key"></i></button>
 <button type="button" user_type="administrador" user_id="<?php echo $v['User']['id'];?>" class="btn btn-danger btn-mini delete-user"><i class="fa fa-trash-o"></i></button>											
									</td>
									<td class="center myclas">
										<?php if($v['User']['active']){ ?>
